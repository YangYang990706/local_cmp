...
| **下级企业客户** | 由 reseller 或 root 通过 API 或 Portal 创建的任意层级下级账户 |
...
**User Story:** AS 账户管理员，I want 管理账户下的用户，so that 不同操作人员可以按授权访问平台。资产和用户都挂载在账户下，资产是被管理的对象，用户是管理资产的执行者。用户在对应账户下被创建并获得权限后，即可对归属账户下的资产执行权限范围内的操作。
...
10. WHEN 用户登录系统，THE 系统 SHALL 根据其归属账户和拥有的权限组确定可操作的数据范围：若用户拥有 ADMIN_ALL 权限且归属 reseller 账户，可操作范围为该 reseller 及其递归下级所有账户的资产和用户；若用户拥有特定 ADMIN_* 权限组（如 ADMIN_SIM），可操作范围同样递归覆盖下级账户的对应类型资产。
11. THE 系统 SHALL 确保资产和用户均挂载在账户下：资产是操作对象，用户是操作执行者。用户在账户下被创建并赋权后，根据拥有的权限组对归属账户及其下级账户的资产执行对应操作。
...
| 权限组编码 | 权限说明 | 适用资产范围 |
|-----------|----------|--------------|
| ADMIN_ALL | 所有权限，全局视图与全部管理能力 | 若用户在 root 账户：ALL（全局）；若用户在 reseller 账户：该 reseller 及其所有下级账户（递归）的全部资产 |
...
| 创建下级企业客户 | N | N | N | N | N | N | N | N | N | N | N | Y | N |
...
6. WHEN 上级账户的用户拥有管理权限（ADMIN_ALL 或特定 ADMIN_* 权限组），THE 系统 SHALL 允许其操作名下所有下级账户（递归）的对应资产。例如：账户 A(reseller) 的 ADMIN_ALL 用户可管理 A→B→C→D 链上所有账户的资产；账户 B(reseller) 的 ADMIN_ALL 用户可管理 B→C→D 链上所有账户的资产。
...
本节定义账户、用户、资产三者的核心关系，是所有需求的基础前提。

```mermaid
graph TD
    subgraph "实体关系"
        Account["账户 (Account)<br/>资产容器<br/>树状层级结构"]
        User["用户 (User)<br/>操作执行者<br/>挂载于账户"]
        Asset["资产 (Asset)<br/>SIM/eSIM/Profile<br/>挂载于账户"]
        Role["权限组 (Role)<br/>赋予用户"]
    end
...
| # | 规则 | 说明 |
|---|------|------|
| 1 | 账户是资产的容器 | 所有资产（SIM/eSIM/Profile）必须挂载在某个具体账户下，不存在无账户归属的资产 |
| 2 | 账户是用户的归属 | 所有用户必须挂载在某个具体账户下，不存在无账户归属的用户 |
| 3 | 用户通过凭据登录 | 登录依赖用户名+密码，不是"账户登录"——账户是数据隔离单元，用户是身份认证单元 |
| 4 | 权限赋予用户 | 权限组（ADMIN_* / read_*）直接关联用户，不同用户可以有不同的权限组合 |
| 5 | 用户操作资产 | 用户登录后，根据拥有的权限组，对归属账户下的资产执行操作 |
| 6 | 上级可管下级 | 上级账户的用户（拥有管理权限）可向下递归管理所有下级账户的资产和用户 |
| 7 | 下级不可向上 | 下级账户的用户仅能操作本账户及以下的数据，不可向上访问 |
| 8 | 用户间平等隔离 | 用户之间无所属/上下级关系，不同账户的用户完全隔离 |
...
账户: 国(root)    用户: 国务院(ADMIN_ALL)、财政部(ADMIN_Billing)、工信部(ADMIN_SIM)
账户: 省(reseller) 用户: 省政府(ADMIN_ALL)
账户: 县(reseller) 用户: 县政府(ADMIN_ALL)
资产: "人"挂载在各级账户下

县政府(ADMIN_ALL) → 可管理县、乡、村的全部"人"（资产）
省政府(ADMIN_ALL) → 可管理省、市、县、乡、村的全部"人"（资产）
```
...
创建流程:
1. 捷德创建A，并为A创建首个ADMIN_ALL用户
2. B可由A(或捷德)创建，parent_account_id=A
3. C可由B(或A或捷德)创建，parent_account_id=B
4. D可由C(或B或A或捷德)创建，parent_account_id=C
   C可要求D不允许再卖给其他人（D=customer不可建下级）

A的ADMIN_ALL用户 → 管理A/B/C/D全部资产
C的ADMIN_ALL用户 → 管理C/D全部资产
```
...
3. THE 系统 SHALL 对一级企业客户开放以下 API 能力：自建下级企业客户、归属资产管理、归属资产业务操作、商用套餐列表查询、话单和账单查询。
...
| V2.3.1 | 2026-07-13 | AI Coding Agent | 明确 ADMIN_ALL 权限递归规则：上级账户的 ADMIN_ALL 用户可管理其所有下级账户（递归）的全部资产。资产和用户均挂载在账户下，用户通过权限管理资产。补充 A→B→C→D 层级示例说明 |
