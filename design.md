...
- 资产归属账户（`Account → Asset`），资产是被管理对象，用户是操作执行者
- 权限赋予用户（`User → Role`），同一账户的不同用户可拥有不同权限组合
- 用户通过拥有的权限对归属账户下的资产执行对应操作
- 拥有 ADMIN_ALL 或特定 ADMIN_* 权限的上级账户用户，可以向下递归管理所有下级账户的资产和用户

**ADMIN_ALL 权限的递归范围：**
```
账户树: A(reseller) → B(reseller) → C(reseller) → D(customer)

A 的 ADMIN_ALL 用户 → 可管理 A, B, C, D 全部资产
B 的 ADMIN_ALL 用户 → 可管理 B, C, D 全部资产
C 的 ADMIN_ALL 用户 → 可管理 C, D 全部资产
